"""Heat pump model with open or closed economizer."""
...
class HeatPumpEcon():
    ...
    def generate_components(self):
        ...
        self.comps['comp_1'] = Compressor('Compressor 1')
        self.comps['merge'] = Merge('Economizer Injection')
        self.comps['comp_2'] = Compressor('Compressor 2')
    ...
    def generate_connections(self):
        ...
        self.conns['A5'] = Connection(
            self.comps['evap'], 'out2', self.comps['comp_1'], 'in1', 'A5'
            )
        self.conns['A6'] = Connection(
            self.comps['comp_1'], 'out1', self.comps['merge'], 'in1', 'A6'
            )
        self.conns['A7'] = Connection(
            self.comps['merge'], 'out1', self.comps['comp_2'], 'in1', 'A7'
            )
        self.conns['A8'] = Connection(
            self.comps['comp_2'], 'out1', self.comps['cond'], 'in1', 'A8'
            )
        ...
        self.busses['power input'].add_comps(
            {'comp': self.comps['comp_1'], 'char': mot, 'base': 'bus'},
            {'comp': self.comps['comp_2'], 'char': mot, 'base': 'bus'},
    ...
    def init_simulation(self, **kwargs):
        ...
        # Components
        self.comps['comp_1'].set_attr(eta_s=self.params['comp_1']['eta_s'])
        self.comps['comp_2'].set_attr(eta_s=self.params['comp_2']['eta_s'])
        ...
        if self.econ_type.lower() == 'closed':
            ...
            self.conns['A2'].set_attr(
                m=Ref(self.conns['A1'], self.params['A2']['m_rel'], 0)
                )
    ...
    def design_simulation(self, **kwargs):
        ...
        if self.nw.res[-1] <= 1e-3:
            ...
            self.design_path = 'hp_econ_design'
            self.nw.save(self.design_path)

        self.m_design = self.conns['A8'].m.val
    ...
    def offdesign_simulation(self):
        ...
        # Parametrization
        self.comps['comp_1'].set_attr(
            design=['eta_s'], offdesign=['eta_s_char']
            )
        self.comps['comp_2'].set_attr(
            design=['eta_s'], offdesign=['eta_s_char']
            )
        ...
        if self.params['offdesign']['save_results']:
            with open('hp_econ_partload.json', 'w') as file:
    ...
    def get_log_ph_states(self):
        ...
        data.update(
            {self.comps['comp_1'].label:
             self.comps['comp_1'].get_plotting_data()[1]}
        )
        data.update(
            {self.comps['comp_2'].label:
             self.comps['comp_2'].get_plotting_data()[1]}
        )
...
if __name__ == '__main__':
    ...
    with open(f'params_hp_econ_{econ_type}_4GDH.json', 'r') as file:
        params = json.load(file)


    hp = HeatPumpEcon(params, econ_type=econ_type)
    ...
    data.to_csv(
        os.path.join(params['fluids']['wf'], f'hp_econ_{econ_type}_4GDH.csv'),
        sep=';'
        )
